...
LOADER="${AI_ROOT}/core/loader/ai_workflow_loader_v2.sh"
# State machine tables live in core/config/state-machine.yaml; this library
# is the only reader. Never fork the tables into this script.
source "${AI_ROOT}/core/lib/state_machine.sh"
...
if [ -z "$ROLE_ID" ]; then ROLE_ID="developer"; fi
...
PROJECT_RULES="$(resolve_input host.instructions)"
...
usage() {
  cat <<EOF
Usage: ai-workflow next [--project-root <path>] <task-file> [stage]
...
Examples:
  ai-workflow next <runtime-task>
  ai-workflow next <runtime-task> verify
EOF
}
...
print_required_context() {
  local request defect_count=0
  printf '%s\n' "  - project.profile"
  printf '%s\n' "  - project.knowledge.index"
  printf '%s\n' "  - runtime.task"
  [ "$CURRENT_DELIVERY_TIER" != "L1" ] || return
  while IFS= read -r request; do
    case "$request" in
      .ai/architecture/*|.ai/risks/*|.ai/policies/*)
        printf '  - %s\n' "$request"
        ;;
      .ai/defects/*)
        [ "$defect_count" -lt 5 ] || continue
        defect_count=$((defect_count + 1))
        printf '  - %s\n' "$request"
        ;;
    esac
  done < <(read_required_context "$TASK_FILE")
}
...
section_field_item() {
  ...
  section_content "$section" "$file" | awk -v field="$field" '
    $0 ~ "^[[:space:]]*-[[:space:]]*" field "(:|：)" {
      ...
      if (value != "" && value !~ /^(待填写|待补充|未知|unknown|n\/a|pending|todo|tbd)$/) {
        print value
        exit
      }
      capture = 1
...
is_substantive() {
  case "$(printf '%s' "$1" | tr '[:upper:]' '[:lower:]')" in
    ""|无|n/a|not_run|pending|待填写|待补充|未知|unknown|todo|tbd)
      return 1
      ;;
    *)
      return 0
...
normalize_stage() {
  sm_normalize_stage "$1"
}

decision_stage_to_loop_stage() {
  sm_decision_to_loop "$1"
}

decision_transition_allowed() {
  [ "$(sm_transition_allowed "$1" "$2" "$3")" = "yes" ]
}

loop_stage_to_decision_stage() {
  sm_loop_to_decision "$1"
}
...
COMMIT_REQUIRED="$(section_field_value "Commit Ready" "commit_required" "$TASK_FILE" | tr '[:upper:]' '[:lower:]')"
if [ -z "$COMMIT_REQUIRED" ]; then
...
if [ -z "$TYPE" ]; then
  echo "Unknown Core task type in ${TASK_FILE}: ${TYPE_INPUT}"
  exit 1
fi
...
if [ "$DELIVERY_LEVEL" = "high_risk_delivery" ]; then
  if [ "$RISK_SIGNALS_PRESENT" != "true" ]; then
    risk_errors+=("high_risk_delivery 必须填写交付风险信号")
  fi
  if [ "$ALIGNMENT_REQUIRED" != "true" ]; then
    risk_errors+=("high_risk_delivery 必须设置 alignment_required=true")
  fi
  if [ "$SELF_TEST_LEVEL" != "specialized" ]; then
    risk_errors+=("high_risk_delivery 的 self_test_level 必须为 specialized")
  fi
fi
...
default_next_stage() {
  sm_default_next "$STATUS" "$DELIVERY_LEVEL" "$ALIGNMENT_STATUS" "$TYPE_REAL_ENV" "$COMMIT_REQUIRED"
}
...
for gate in "L0:${L0}" "L1:${L1}" "L2:${L2}" "L3:${L3}" "HighRisk:${HIGH_RISK}" "Historical:${HISTORICAL_GATE}"; do
  name="${gate%%:*}"
  value="${gate#*:}"
  case "$(gate_result_kind "$value")" in
    pass|na)
      ;;
    pass_with_risk)
      warning_gates+=("$name")
...
case "$DELIVERY_LEVEL" in
  micro_change|light_feature|standard_delivery|high_risk_delivery)
    for gate_name in $(sm_required_gates "$DELIVERY_LEVEL"); do
      case "$gate_name" in
        L0) add_required_gate "L0" "$L0" ;;
        L1) add_required_gate "L1" "$L1" ;;
        L2) add_required_gate "L2" "$L2" ;;
        L3) add_required_gate "L3" "$L3" ;;
        DiffReviewLite) add_required_gate "DiffReviewLite" "$DIFF_REVIEW_LITE" ;;
        HighRisk) add_required_gate "HighRisk" "$HIGH_RISK" ;;
        Architecture) add_required_gate "Architecture" "$ARCH_GATE" ;;
      esac
    done
    if [ "$DELIVERY_LEVEL" = "high_risk_delivery" ]; then
...
SELF_TEST_TRAFFIC_LIGHT="$(section_content "Self Test 红绿灯" "$TASK_FILE")"
if [ -z "$SELF_TEST_TRAFFIC_LIGHT" ]; then
  required_missing+=("SelfTestTrafficLight")
  TRAFFIC_LIGHT_MISSING="true"
else
  TRAFFIC_LIGHT_MISSING="false"
fi
...
if [ "$TRAFFIC_LIGHT_MISSING" = "true" ] && { [ "$STATUS" = "self_test_in_progress" ] || [ "$STATUS" = "review" ] || [ "$STATUS" = "ready_for_qa" ]; }; then
  forbidden_reasons+=("review/ready_for_qa：缺少 Self Test 红绿灯证据")
fi

if [ "$ITERATION_LIMIT_HIT" = "true" ]; then
  forbidden_reasons+=("iteration ${ITERATION}/${MAX_ITERATIONS} 超过上限，必须 BLOCKED")
fi
...
case "$STATUS" in
  ...
  alignment_pending)
    ...
    ;;
  fix_ready)
    allowed=("fix_ready" "implement" "impact_map" "blocked")
    ;;
  implement)
    allowed=("implement" "ready_for_self_test" "blocked")
    ;;
  ready_for_self_test)
    allowed=("ready_for_self_test" "self_test_in_progress" "implement" "blocked")
    ;;
  validation_pending)
    allowed=("validation_pending" "self_test_in_progress" "implement" "blocked")
    ;;
  self_test_in_progress|verify)
    ...
    ;;
  review)
    allowed=("review" "implement" "ready_for_qa" "commit_ready" "done" "blocked")
    if [ "${#required_missing[@]}" -gt 0 ] || [ "${#required_failed[@]}" -gt 0 ] || [ "${#required_blocked[@]}" -gt 0 ]; then
    ...
    ;;
  i18n)
    allowed=("i18n" "review" "commit_ready" "blocked")
    ;;
  ready_for_qa)
    allowed=("ready_for_qa" "done" "commit_ready" "review")
    ;;
  commit_ready)
    allowed=("commit_ready" "done")
    ;;
  done)
    allowed=("done" "commit_ready")
    ;;
  blocked)
...
if [ -n "$TYPE_DEFAULT_DELIVERY" ] && [ "$DELIVERY_LEVEL" != "$TYPE_DEFAULT_DELIVERY" ] && [ "$DELIVERY_LEVEL" != "missing" ]; then
  forbidden_reasons+=("delivery_level：${TYPE} 任务必须使用 Core 配置的 ${TYPE_DEFAULT_DELIVERY}")
fi
...
if [ "$STAGE" = "commit_ready" ] && [ "$STATUS" != "review" ] && [ "$STATUS" != "done" ] && [ "$STATUS" != "ready_for_qa" ] && [ "$STATUS" != "commit_ready" ]; then
...
case "$STAGE" in
  ...
  analyze)
    case "$TYPE" in
      ...
      *) AGENT="requirement"; AGENT_FILE="$AGENT_REQUIREMENT"; ACTION="按 Core 对 ${TYPE} 的定义分析真实链路、能力差异、回归范围和需要确认的问题，不改代码，并回填任务卡。" ;;
    esac
    MODE_NEXT="plan"
  ...
  commit_ready)
    ...
    MODE_NEXT="review"
    ACTION="进入 COMMIT_READY。Commit Agent 只基于当前 diff、task card、自测结果、PR Review 结论、Company Policy 和仓库规则生成可复制 Commit 信息；不得执行 git add / git commit / git push，不得修改暂存内容，等待人工手动提交。"
    PASS_CONDITION="Commit 信息与实际 diff、QA 结论、Company Policy 和仓库规则一致，并明确是否满足提交前置条件。"
    ;;
  blocked)
    AGENT="router"
...
## Context Assembly
- context_budget: ${CONTEXT_BUDGET_ID}
- required_context:
$(print_required_context)
...
- context_budget / required_context；只列本阶段确实需要的 Project Local Knowledge 文件
- 证据记录（Evidence）
...
- 需求规则无法确认。
- 无法完成关键验证。
- 影响 Project Local Knowledge 声明的高风险平台能力、transport、升级或数据链路。
- 影响公共基础组件且回归范围扩大。

收尾要求：
- 最终回复末尾必须输出 "## Loop Closeout"。
- 完成当前阶段后，必须把 Loop Closeout 写回任务卡，并调用 ai-workflow finish 收口。
- "当前状态" 必须写本阶段结束后的真实状态。
- "下一阶段" 必须写本阶段结束后的唯一下一阶段；只有 Done 时可写“无”。
- "是否 Done" 只能写“是”或“否”。
...
ai-workflow assemble --project-root ${PROJECT_ROOT} ${AGENT} ${TASK_FILE}
EOF
